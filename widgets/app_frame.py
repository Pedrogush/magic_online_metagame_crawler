...
class AppFrame(AppEventHandlers, SideboardGuideHandlers, CardTablePanelHandler, wx.Frame):
    ...
    def _handle_increment_shortcut(self) -> bool:
    ...
    def _handle_decrement_shortcut(self) -> bool:
        selection = self._get_selected_zone_card()
    ...
    def _get_selected_zone_card(self) -> tuple[str, dict[str, Any]] | None:
        for zone, table in (
    ...
    def _get_selected_search_card(self) -> dict[str, Any] | None:
        if not self.builder_panel:
            return None
        return self.builder_panel.get_selected_result()

    def _get_active_zone_for_add(self) -> str:
    ...
    def _get_table_for_zone(self, zone: str) -> CardTablePanel | None:
        if zone == "main":
