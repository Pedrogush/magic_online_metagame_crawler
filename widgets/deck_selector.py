...
class MTGDeckSelectionFrame(
    ...
    def __init__(self, parent: wx.Window | None = None):
        ...
        self.app_state = self.state_store.build_deck_selector_state(
            self.settings,
            format_options=FORMAT_OPTIONS,
            default_format="Modern",
            default_bulk_days=int(DEFAULT_BULK_DATA_MAX_AGE_DAYS),
            min_bulk_days=BULK_CACHE_MIN_AGE_DAYS,
            max_bulk_days=BULK_CACHE_MAX_AGE_DAYS,
        )
    ...
    def _build_left_panel(self, parent: wx.Window) -> wx.Panel:
        ...
        self.research_panel = DeckResearchPanel(
            ...
            format_options=FORMAT_OPTIONS,
            initial_format=self.app_state.current_format,
        ...
        self._show_left_panel(self.app_state.left_mode, force=True)
    ...
    def _show_left_panel(self, mode: str, force: bool = False) -> None:
        ...
        if force or self.app_state.left_mode != target:
            self.app_state.left_mode = target
            self.settings["left_mode"] = target
    ...
    def _restore_session_state(self) -> None:
        ...
        if saved_mode in {"research", "builder"}:
            self.app_state.left_mode = saved_mode
            self._show_left_panel(self.app_state.left_mode, force=True)
    ...
    def _save_window_settings(self) -> None:
        ...
        data.update(
            {
                "format": self.app_state.current_format,
                "window_size": [size.width, size.height],
                "screen_pos": [pos.x, pos.y],
                "left_mode": self.app_state.left_mode,
                "force_cached_bulk_data": self.app_state.force_cached_bulk_data,
                "bulk_data_max_age_days": self.app_state.bulk_data_max_age_days,
    ...
    def _get_bulk_cache_age_days(self) -> int:
        return self.app_state.bulk_data_max_age_days

    def _is_forcing_cached_bulk_data(self) -> bool:
        return self.app_state.force_cached_bulk_data

    def _set_force_cached_bulk_data(self, enabled: bool) -> None:
        if self.app_state.force_cached_bulk_data == enabled:
            return
        self.app_state.force_cached_bulk_data = enabled
    ...
    def _set_bulk_cache_age_days(self, days: int) -> None:
        clamped = self.state_store.clamp_bulk_cache_age(
            days,
            default_days=self.app_state.bulk_data_max_age_days,
            min_days=BULK_CACHE_MIN_AGE_DAYS,
            max_days=BULK_CACHE_MAX_AGE_DAYS,
        )
        if clamped == self.app_state.bulk_data_max_age_days:
            return
        self.app_state.bulk_data_max_age_days = clamped
    ...
    def fetch_archetypes(self, force: bool = False) -> None:
        with self._loading_lock:
            if self.app_state.loading_archetypes:
                return
            self.app_state.loading_archetypes = True
        self._set_status(f"Loading archetypes for {self.app_state.current_format}…")
    ...
    def _load_decks_for_archetype(self, archetype: dict[str, Any]) -> None:
        with self._loading_lock:
            if self.app_state.loading_decks:
                return
            self.app_state.loading_decks = True
    ...
    def _start_daily_average_build(self) -> None:
        ...
        with self._loading_lock:
            self.app_state.loading_daily_average = True
        self.daily_average_button.Disable()
        ...
        def on_success(buffer: dict[str, float]):
            ...
            with self._loading_lock:
                self.app_state.loading_daily_average = False
