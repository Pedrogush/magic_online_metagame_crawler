...
from navigators.mtgo import load_box_positions
...
def get_bottom_leaderboard(count=999):
    ...
    while True:
        ...
        trophy_counts = [
            get_player_trophy_count(b, "black")
            for b in BOX_POSITIONS["bottom"].values()
        ]
        ...
        if not new_player_found:
            break
        scroll_down(
            BOX_POSITIONS["bottom"]["1"][0][0], BOX_POSITIONS["bottom"]["1"][0][1]
        )
        scrolls += 1
...
def update_db_leaderboard(format="Modern"):
    ...
    db.leaderboard.insert_one(
        {"leaderboard": leaderboard, "scrape_ts": scrape_ts, "format": format}
    )
    return leaderboard
...
def update_db_metagame_trophy_share(format="Modern"):
    ...
    db.metagame_trophy_share.insert_one(
        {"metagame": metagame, "scrape_ts": time.time(), "format": format}
    )
    return metagame
