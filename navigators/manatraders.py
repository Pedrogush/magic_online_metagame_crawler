...
def wait_for_confirm_rent_dialog(driver: webdriver.Chrome):
    start_time = time.time()
    confirm_rent_dialog = driver.find_element(
        by=By.ID, value="insert-username-for-rent-modal"
    )
...
def wait_for_bot_to_load_cards(driver: webdriver.Chrome):
    ...
    start_time = time.time()
    bot_loading = driver.find_element(
        by=By.XPATH,
        value='//*[@id="page-wrapper"]/div[1]/div[1]/div[1]/div[1]/form/button',
    )  # noqa
...
def wait_for_bot_to_be_ready_to_request_cards(driver: webdriver.Chrome):
    ...
    start_time = time.time()
    bot_ready = driver.find_element(
        by=By.XPATH, value='//*[@id="page-wrapper"]/div[3]/div/div/div[1]/form/button'
    )
...
def login(driver: webdriver.Chrome, achains: webdriver.ActionChains):
    ...
    achains.move_to_element(email_box).click().send_keys(
        config["manatraders_login"]
    ).perform()
    password_box = driver.find_element(by=By.ID, value="user_password")
    achains.move_to_element(password_box).click().send_keys(
        config["manatraders_pass"]
    ).perform()
...
def rent_deck(
    driver: webdriver.Chrome, achains: webdriver.ActionChains, deck_num: str = ""
):
    if deck_num:
    ...
    upload_btn = driver.find_element(
        by=By.XPATH, value='//*[@id="page-wrapper"]/div[2]/div/div[2]/div[4]/button[2]'
    )  # noqa
    ...
    achains.move_to_element(text_area).click().click().key_down(Keys.CONTROL).send_keys(
        "v"
    ).key_up(
        Keys.CONTROL
    ).perform()  # noqa
    ...
    rent_button = driver.find_element(
        by=By.XPATH, value='//*[@id="page-wrapper"]/div[2]/div/div[2]/div[2]/div/button'
    )  # noqa
    achains.move_to_element(rent_button).click().click().perform()
    wait_for_confirm_rent_dialog(driver)
    time.sleep(2)
    confirm_rent_button = driver.find_element(
        by=By.XPATH,
        value='//*[@id="insert-username-for-rent-modal"]/div/div/div[3]/button[2]',
    )  # noqa
...
def return_cards(driver: webdriver.Chrome, action_chains: webdriver.ActionChains):
    ...
    return_rental_button = driver.find_element(
        by=By.XPATH,
        value='//*[@id="page-wrapper"]/div[2]/div[2]/div[2]/div/div/div/div[2]/div[1]/button',
    )  # noqa
    action_chains.move_to_element(return_rental_button).click().click().perform()
    wait_for_rent_return_menu_fade_in(driver)
    return_cards_btn = driver.find_element(
        by=By.CSS_SELECTOR, value='input[value="Return cards"]'
    )
