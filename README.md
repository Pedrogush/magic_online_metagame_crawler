...
This repository includes a lightweight .NET bridge (`dotnet/MTGOBridge`) that pulls data from the MTGOSDK and feeds it to the Python tooling. Follow the steps below to install the required tooling, add the MTGOSDK package, and build the bridge executable.

## 1. Install the .NET SDK (Windows)

1. Download and install [.NET 10 SDK or newer](https://dotnet.microsoft.com/download/dotnet/10.0).
...
> **Tip:** If you already have Visual Studio 2026 (v18+) installed with .NET tooling, the SDK may already be available.

## 2. Restore dependencies and add MTGOSDK

From the repository root (`C:\Users\<you>\Documents\GitHub\magic_online_metagame_crawler`):
